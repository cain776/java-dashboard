...
REG AS (
  SELECT '20'+SUBSTRING(R.RESERVE_NUM,1,2)+'-'+SUBSTRING(R.RESERVE_NUM,3,2)+'-'+SUBSTRING(R.RESERVE_NUM,5,2) AS d,
         SUM(CASE WHEN R.RESERVE_PATH='APP' AND R.RESERVE_STATE<>'C' THEN 1 ELSE 0 END) AS appReservation,
         SUM(CASE WHEN R.RESERVE_PATH='APP' AND R.RESERVE_STATE ='C' THEN 1 ELSE 0 END) AS appCancel,
         SUM(CASE WHEN R.RESERVE_PATH='CRM' AND R.RESERVE_STATE<>'C' THEN 1 ELSE 0 END) AS crmReservation,
