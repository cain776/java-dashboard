...
import com.bviit.analytics.dto.reservation.ReservationOverallMonthlyItem;
import com.bviit.analytics.repository.reservation.ReservationOverallStatsRepository;
 ...
 *       · 온라인 = RESERVE_PATH ONLINE/APP(홈페이지) + NAVER(네이버). **카카오 미포함**.
 *       · 종합  = 콜 + 온라인. 카카오(해피톡)는 레거시가 월마다 포함/미포함이 들쭉날쭉해 종합에서 제외
 *                (예약자 리스트에서만 참고 표시).
