...
> **배포 문서 3종 — 읽는 순서**: ① **전략·CI(이 문서)** → ② [로컬 개발 실행](deployment-local.md) → ③ [운영 배포](deployment-release.md)

## 목표

`java_dashboard`는 소규모 내부 사용 제품이므로, 복잡한 완전 자동 배포보다 `자동 검증 + 수동 배포 승인` 전략을 기본으로 사용합니다.
