...
> ⚠️ 위 ✅/🟡/🟥는 **API 준비도**(데이터 소스 존재 여부) 기준이다. 아래는 **실제 `MonthlyReportPage`가 렌더하는 데이터원**이다.
...
- **직장인 검사 / 학생 검사 / 기타 검사** *(`overall-exam/weekly` → `jobOffice/Student/Etc`, prod 검증)*
...
| 4 일반 고객 검사 | 1123·915·584·559 | 1307·1053·635·608 | **106건 ⚠️정의차** |
| 5 고객소개 검사 | 745·650·335·303 | 559·501·282·253 | **110건 ⚠️정의차** |
...
> 신규(5월부터 자동 집계): 2026-05 — 직장인 654·학생 189·기타 211·직원소개 76·일반고객 654·고객소개 324 / 일반검사비율 ~59%·원데이예약률 91%·중단율 6%.
...
- **#4·5 소개유형(일반고객·고객소개) ~106~110건 차이 — 정의차(버그 아님)**: 우리 DB는 고객소개=`MOTIVE_NEW02 소개고객`만 집계해 레거시 PDF보다 고객소개를 적게(−110)·일반을 많게(+106) 분류. **총검사자 합계는 일치**(1월 일반 1307 + 고객 559 + 직원 169 = 2035 ≈ 레거시 2038). 직원소개·직업(직장인/학생/기타)은 ≤3건으로 거의 일치. 소개유형 정의 합의 시 수렴.
